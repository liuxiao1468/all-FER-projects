...
def extract_AU(xlist,ylist):
    ...
    AU13_1_x = xlist[0:6]
    AU13_1_y = ylist[0:6]
    AU13_1_x,AU13_1_y = linear_interpolation(AU13_1_x,AU13_1_y)
    AU13_1_x,AU13_1_y = linear_interpolation(AU13_1_x,AU13_1_y)
    AU_feature = AU_feature + get_average_curvature(AU13_1_x,AU13_1_y)

    AU13_2_x = xlist[11:17]
    AU13_2_y = ylist[11:17]
    AU13_2_x,AU13_2_y = linear_interpolation(AU13_2_x,AU13_2_y)
    AU13_2_x,AU13_2_y = linear_interpolation(AU13_2_x,AU13_2_y)
    AU_feature = AU_feature + get_average_curvature(AU13_2_x,AU13_2_y)

    AU17_x = xlist[6:11]
    AU17_y = ylist[6:11]
    AU17_x,AU17_y = linear_interpolation(AU17_x,AU17_y)
    AU17_x,AU17_y = linear_interpolation(AU17_x,AU17_y)
    AU_feature = AU_feature + get_average_curvature(AU17_x,AU17_y)
...
def get_vectorized_landmark(image):
    ...
    for k,d in enumerate(detections): #For all detected face instances individually
        ...
        # landmarks_dist = landmarks_dist[:]
        # landmarks_theta = landmarks_theta[:]

        # landmarks_dist = landmarks_dist[0:17]+landmarks_dist[17:27]+landmarks_dist[31:40]+ landmarks_dist[42:51]+ [landmarks_dist[60]]+[landmarks_dist[67]]+ [landmarks_dist[64]]+ [landmarks_dist[65]]+ landmarks_dist[52:60]
        # landmarks_theta = landmarks_theta[0:17]+landmarks_theta[17:27]+landmarks_theta[31:40]+ landmarks_theta[42:51]+ [landmarks_theta[60]]+[landmarks_theta[67]]+ [landmarks_theta[64]]+ [landmarks_theta[65]]+ landmarks_theta[52:60]
        landmarks_dist = landmarks_dist[0:17]+[landmarks_dist[60]]+[landmarks_dist[67]]+ [landmarks_dist[64]]+ [landmarks_dist[65]]+ landmarks_dist[52:60]
        landmarks_theta = landmarks_theta[0:17]+ [landmarks_theta[60]]+[landmarks_theta[67]]+ [landmarks_theta[64]]+ [landmarks_theta[65]]+ landmarks_theta[52:60]
...
########### use this to change ONLY VL/ ONLY AU ################################################
# def make_training_sets(w1,w2,k):
...
#                 # vec_AU = k*((vec_AU-np.min(vec_AU))/np.ptp(vec_AU))
...
for m in range(0,1):
    ...
    [training_data, training_labels, prediction_data, prediction_labels] = make_training_sets(w1,w2,kk)
