...
class training_model():
    ...
    def extract_AU(self,xlist,ylist,n_landmark):
        AU_feature = []
        Norm_AU_feature = []
        ...
        # eyes+mouth+eyebrow (AU 1,2,5,7,10,12,20)
        if n_landmark == 39:
            AU1_1_x = xlist[19:22]
            AU1_1_y = ylist[19:22]
        ...
        # eyes+mouth+eyebrow+nose (AU 1,2,5,7,9,10,12,20)
        if n_landmark == 44:
            AU1_1_x = xlist[19:22]
            AU1_1_y = ylist[19:22]
            AU1_1_x,AU1_1_y = self.linear_interpolation(AU1_1_x,AU1_1_y)
    ...
    def get_vectorized_landmark(self,image,n_landmark):
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
        clahe_image = clahe.apply(gray)
        detections = self.detector(clahe_image, 1)
        for k,d in enumerate(detections): #For all detected face instances individually
            ...
            # eyes (AU 5,7)
            if n_landmark == 13:
                landmarks_dist = landmarks_dist[36:40]+ landmarks_dist[42:46]+ landmarks_dist[39:42]+ landmarks_dist[46:48]
                landmarks_theta = landmarks_theta[36:40]+ landmarks_theta[42:46]+ landmarks_theta[39:42]+ landmarks_theta[46:48]

            # mouth (AU 10,12,20)
            if n_landmark == 15:
                landmarks_dist = landmarks_dist[48:51]+ landmarks_dist[52:60]+ [landmarks_dist[60]]+ [landmarks_dist[67]]+[landmarks_dist[64]]+[landmarks_dist[65]]
                landmarks_theta = landmarks_theta[48:51]+ landmarks_theta[52:60]+ [landmarks_theta[60]]+ [landmarks_theta[67]]+[landmarks_theta[64]]+[landmarks_theta[65]]

            # eyes+mouth (AU 5,7,10,12,20)
            if n_landmark == 28:
                landmarks_dist = landmarks_dist[36:40]+ landmarks_dist[42:46]+ landmarks_dist[46:51]+ [landmarks_dist[60]]+[landmarks_dist[67]]+ [landmarks_dist[64]]+ [landmarks_dist[65]]+ landmarks_dist[52:60]
                landmarks_theta= landmarks_theta[36:40]+ landmarks_theta[42:46]+ landmarks_theta[46:51]+ [landmarks_theta[60]]+[landmarks_theta[67]]+ [landmarks_theta[64]]+ [landmarks_theta[65]]+ landmarks_theta[52:60]

            # eyes+mouth+eyebrow (AU 1,2,5,7,10,12,20)
            if n_landmark == 39:
                landmarks_dist = landmarks_dist[17:27]+landmarks_dist[36:40]+ landmarks_dist[42:51]+ [landmarks_dist[60]]+[landmarks_dist[67]]+ [landmarks_dist[64]]+ [landmarks_dist[65]]+ landmarks_dist[52:60]
                landmarks_theta= landmarks_theta[17:27]+landmarks_theta[36:40]+ landmarks_theta[42:51]+ [landmarks_theta[60]]+[landmarks_theta[67]]+ [landmarks_theta[64]]+ [landmarks_theta[65]]+ landmarks_theta[52:60]
    ...
    def make_training_sets(self,w1,w2,training_percentage,n_landmark):
        ...
        for emotion in self.emotions:
            ...
            for item in training:
                ...
                vec_landmark = self.get_vectorized_landmark(image,n_landmark)*w1
                if (xlist.size) and (vec_landmark.size):
                    Norm_AU_feature = self.extract_AU(xlist,ylist,n_landmark)*w2
    ...
    def __init__(self,w1,making_sets,C,training_percentage,n_landmark):
        ...
        #self.emotions = ["anger",'disgust'] #Emotion list
        ...
        for m in range(0,making_sets):
            ...
            [training_data, training_labels, prediction_data, prediction_labels] = self.make_training_sets(w1,w2,training_percentage,n_landmark)
            ...
            if making_sets < 2:
                ...
                # save the model
                joblib.dump(clf, 'landmark_SVM.pkl')
...
# np.savetxt('/home/leo/woody_vision/last_try/'+'C'+str(i)+'acc'+'.txt', accur_lin, fmt='%1.4e')
