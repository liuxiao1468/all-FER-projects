...
class training_model():
    ...
    def make_training_sets(self,training_percentage,n_landmark):
        ...
        for emotion in self.emotions:
            ...
            for item in training:
                ...
                #vec_landmark = self.get_vectorized_landmark(image)*w1
                if (xlist.size):
                    Norm_AU_feature = self.extract_AU(xlist,ylist,n_landmark)
                    #vec_AU = np.concatenate((Norm_AU_feature,vec_landmark))
                    vec_AU = ((Norm_AU_feature-np.min(Norm_AU_feature))/np.ptp(Norm_AU_feature))
    ...
    def __init__(self,making_sets,C,training_percentage,n_landmark):
        ...
        #self.emotions = ["anger",  "happiness","neutral"] #Emotion list
        ...
        for m in range(0,making_sets):
            ...
            [training_data, training_labels, prediction_data, prediction_labels] = self.make_training_sets(training_percentage,n_landmark)
            ...
            if making_sets < 2:
                ...
                # save the model
                joblib.dump(clf, 'landmark_SVM_AU.pkl')
            else:
                if m == 0:
                    self.p_pred_lin = pred_lin
                    joblib.dump(clf, 'landmark_SVM_AU.pkl')
